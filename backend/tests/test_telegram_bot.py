...
from app.bot.telegram.formatters import hall_of_fame as hall_management_fmt
...
def test_superadmin_calendar_month_uses_codes_without_registration_counts() -> None:
    view = TournamentCalendarMonthView(
        ...
        tournament_types=(
            TournamentCalendarMonthTypeView(
                id=1, name="Баунти турнир", short_name="Баунти", calendar_code="B"
            ),
            TournamentCalendarMonthTypeView(
                id=7,
            ...
            TournamentCalendarMonthTypeView(
                id=8, name="Bounty", short_name="Bounty", calendar_code="B2"
            ),
            TournamentCalendarMonthTypeView(
                id=9, name="Classic", short_name="Classic", calendar_code="C2"
            ),
            TournamentCalendarMonthTypeView(
                id=10, name="Freezeout", short_name="Freezeout", calendar_code="F2"
            ),
            TournamentCalendarMonthTypeView(
                id=11, name="Deep Stack", short_name="Deep Stack", calendar_code="D"
            ),
            TournamentCalendarMonthTypeView(
                id=12,
                name="White Party Tournament",
                short_name="White Party",
                calendar_code="WP",
            ),
        ),
    )
...
def test_calendar_format_help_distinguishes_types_with_same_short_name() -> None:
    view = TournamentCalendarMonthView(
        year=2026,
        month=9,
        weeks=(),
        tournament_types=(
            TournamentCalendarMonthTypeView(
                id=1, name="Классика 2025", short_name="Классика", calendar_code="C"
            ),
            TournamentCalendarMonthTypeView(
                id=2, name="Классика 2026", short_name="Классика", calendar_code="C2"
            ),
        ),
    )

    keyboard = superadmin_tournaments_kb.calendar_format_help_keyboard(view)
    ...
    assert (
        keyboard.inline_keyboard[0][0].callback_data != keyboard.inline_keyboard[1][0].callback_data
    )


def test_user_schedule_button_does_not_gain_calendar_code() -> None:
    tournament = TournamentView(
        id=1,
        date=date(2026, 9, 23),
    ...
    keyboard = user_tournaments_kb.tournament_schedule_keyboard([tournament])

    assert keyboard.inline_keyboard[0][0].text == "Среда, 23 сентября — Классика"
    assert "C2" not in keyboard.inline_keyboard[0][0].text
...
def test_superadmin_calendar_format_help_keyboard_clamps_out_of_range_pages() -> None:
    view = TournamentCalendarMonthView(
        ...
        tournament_types=tuple(
            TournamentCalendarMonthTypeView(
                id=index, name=f"Type {index}", short_name=f"Type {index}", calendar_code="T"
            )
...
def test_superadmin_calendar_format_help_keyboard_omits_pagination_for_single_page() -> None:
    view = TournamentCalendarMonthView(
        ...
        tournament_types=(
            TournamentCalendarMonthTypeView(
                id=1, name="Bounty", short_name="Bounty", calendar_code="B"
            ),
            TournamentCalendarMonthTypeView(
                id=2, name="Classic", short_name="Classic", calendar_code="C"
            ),
        ),
    )
...
class MutableState:
    ...
    async def _get_state(self) -> object | None:
        return self.state
...
async def test_hall_of_fame_season_with_one_photo_uses_captioned_photo() -> None:
    message = SimpleNamespace(
        answer=AsyncMock(),
        answer_photo=AsyncMock(),
        answer_media_group=AsyncMock(),
...
async def test_hall_of_fame_season_with_two_photos_uses_captioned_collage(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    message = SimpleNamespace(
        answer=AsyncMock(),
    ...
    collage = object()
    build_collage = AsyncMock(return_value=collage)
    monkeypatch.setattr(user_hall_of_fame_handlers, "build_photo_collage", build_collage)

    await user_hall_of_fame_handlers._send_hall_of_fame_season(message, season)

    build_collage.assert_awaited_once_with(message.bot, ["champion-photo", "knockout-photo"])
    message.answer.assert_not_awaited()
    message.answer_photo.assert_awaited_once_with(
        collage,
        caption="Лето 2026\n\n💍 Иван\n💥 Петр",
...
def test_superadmin_hall_of_fame_season_list_paginates_after_five_items() -> None:
    seasons = [
        HallOfFameSeasonListItemView(
            season_id=index,
            season_name=f"Сезон {index}",
            starts_at=date(2026, index, 1),
            ends_at=None,
        )
        for index in range(1, 7)
    ]

    first = pagination_service.paginate(seasons, page=0, page_size=5)
    second = pagination_service.paginate(seasons, page=1, page_size=5)

    assert inline_keyboard_texts(superadmin_hall_of_fame_kb.seasons_keyboard(first)) == [
        "Сезон 1",
        "Сезон 2",
        "Сезон 3",
        "Сезон 4",
    ...
    assert inline_keyboard_texts(superadmin_hall_of_fame_kb.seasons_keyboard(second)) == [
        "Сезон 6",
        "⬅️",
        "2/2",
        "❌ Отмена",
...
async def test_hall_of_fame_today_date_starts_existing_player_search(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    state = MutableState()
    ...
    await superadmin_hall_of_fame_handlers.select_hall_of_fame_date_action(
        ...
        superadmin_hall_of_fame_kb.HallOfFameDateCallback(
            action=superadmin_hall_of_fame_kb.HallOfFameDateAction.TODAY,
            season_id=1,
...
async def test_hall_of_fame_delete_all_photos_requires_confirmation(
    ...
    await superadmin_hall_of_fame_handlers.select_hall_of_fame_photo_action(
        callback,
        superadmin_hall_of_fame_kb.HallOfFamePhotoCallback(
            action=superadmin_hall_of_fame_kb.HallOfFamePhotoAction.DELETE_ALL,
            season_id=1,
        ),
        state,
    )
    service.delete_all_photos.assert_not_awaited()
    assert message.edit_text.await_args.args[0] == "Удалить все фотографии этого сезона?"

    await superadmin_hall_of_fame_handlers.select_hall_of_fame_photo_action(
        callback,
        superadmin_hall_of_fame_kb.HallOfFamePhotoCallback(
            action=superadmin_hall_of_fame_kb.HallOfFamePhotoAction.CONFIRM_DELETE_ALL,
            season_id=1,
        ),
        state,
    )
...
async def test_hall_of_fame_manual_date_player_selection_and_save_deactivate_keyboards(
    ...
    empty_entry = HallOfFameEntryView(
        season_id=1,
        season_name="Осень 2026",
        starts_at=date(2026, 9, 1),
        ends_at=None,
    ...
    await state.update_data(
        hall_season_id=1,
        hall_field=superadmin_hall_of_fame_kb.HallOfFameField.GRAND_KNOCKOUT.value,
        hall_page=1,
        hall_date_prompt_chat_id=123,
        hall_date_prompt_message_id=77,
    )
    player_prompt = SimpleNamespace(chat=SimpleNamespace(id=123), message_id=78)
    input_message = SimpleNamespace(
        from_user=SimpleNamespace(id=123),
        text="24.09.2026",
        chat=SimpleNamespace(id=123),
        bot=SimpleNamespace(edit_message_reply_markup=AsyncMock()),
        answer=AsyncMock(return_value=player_prompt),
    )

    await superadmin_hall_of_fame_handlers.enter_hall_of_fame_awarded_at(input_message, state)

    input_message.bot.edit_message_reply_markup.assert_awaited_once_with(
        chat_id=123,
        message_id=77,
        reply_markup=None,
    )
    ...
    await state.update_data(hall_query="Иван")
    ...
    await superadmin_hall_of_fame_handlers.confirm_hall_of_fame_player(
        confirmation_callback,
        superadmin_hall_of_fame_kb.HallOfFameConfirmCallback(
            action=superadmin_hall_of_fame_kb.HallOfFameConfirmAction.CONFIRM,
            season_id=1,
            field=superadmin_hall_of_fame_kb.HallOfFameField.GRAND_KNOCKOUT,
            player_id=10,
        ),
...
async def test_hall_of_fame_delete_flow_targets_exact_occurrence(
    ...
    service = SimpleNamespace(
        get_season_hall_of_fame=AsyncMock(return_value=entry),
        delete_achievement=AsyncMock(return_value=refreshed),
    )
    monkeypatch.setattr(
        superadmin_hall_of_fame_handlers,
        "hall_of_fame_management_service",
    ...
    await state.update_data(hall_page=1)
    message = SimpleNamespace(
    ...
    await superadmin_hall_of_fame_handlers.select_hall_of_fame_delete_action(
        callback,
        superadmin_hall_of_fame_kb.HallOfFameDeleteCallback(
            action=superadmin_hall_of_fame_kb.HallOfFameDeleteAction.OPEN,
            season_id=1,
            achievement_id=42,
        ),
        state,
    ...
    await superadmin_hall_of_fame_handlers.select_hall_of_fame_delete_action(
        callback,
        superadmin_hall_of_fame_kb.HallOfFameDeleteCallback(
            action=superadmin_hall_of_fame_kb.HallOfFameDeleteAction.CONFIRM,
            season_id=1,
            achievement_id=42,
        ),
        state,
...
async def test_superadmin_hall_of_fame_opens_card_and_searches_candidate(
    ...
    await state.update_data(
        hall_season_id=1,
        hall_field=superadmin_hall_of_fame_kb.HallOfFameField.RATING_WINNER.value,
        hall_awarded_at=date(2026, 8, 31).isoformat(),
    )
...
async def test_hall_of_fame_photo_prompt_opens_and_collects_confirmation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    entry = HallOfFameEntryView(
        season_id=1,
    ...
    source_message = SimpleNamespace(edit_text=AsyncMock())
    callback = SimpleNamespace(
        from_user=SimpleNamespace(id=123),
    ...
    await superadmin_hall_of_fame_handlers.select_hall_of_fame_photo_action(
        callback,
        superadmin_hall_of_fame_kb.HallOfFamePhotoCallback(
            action=superadmin_hall_of_fame_kb.HallOfFamePhotoAction.OPEN_ADD,
            season_id=1,
        ),
        state,
    )
    ...
    assert source_message.edit_text.await_args.args[0] == (
...
def test_superadmin_hall_of_fame_card_and_submenus() -> None:
    champion = UserView(
        id=10,
        telegram_id=None,
        display_name="Иван",
...
def test_hall_of_fame_back_callbacks_preserve_one_level_navigation() -> None:
    entry = HallOfFameEntryView(
        season_id=1,
        season_name="Лето 2026",
        starts_at=date(2026, 6, 1),
    ...
    assert "🗑 Удалить награду" not in inline_keyboard_texts(
        superadmin_hall_of_fame_kb.achievements_keyboard(entry=entry, page=2)
    )

    season_back = superadmin_hall_of_fame_kb.HallOfFameCardCallback.unpack(
        superadmin_hall_of_fame_kb.season_card_keyboard(entry=entry, page=2)
        .inline_keyboard[-2][0]
        .callback_data
    )
    menu_back = superadmin_hall_of_fame_kb.HallOfFameCardCallback.unpack(
        superadmin_hall_of_fame_kb.achievements_keyboard(entry=entry, page=2)
        .inline_keyboard[-2][0]
        .callback_data
    )
    date_back = superadmin_hall_of_fame_kb.HallOfFameDateCallback.unpack(
        superadmin_hall_of_fame_kb.date_prompt_keyboard(
            season_id=1, field=field, today_label="24.09.2026"
        )
        .inline_keyboard[-2][0]
        .callback_data
    )
    name_back = superadmin_hall_of_fame_kb.HallOfFameSearchCallback.unpack(
        superadmin_hall_of_fame_kb.search_prompt_keyboard(season_id=1, field=field)
        .inline_keyboard[-2][0]
        .callback_data
    )
    results_back = superadmin_hall_of_fame_kb.HallOfFameSearchCallback.unpack(
        superadmin_hall_of_fame_kb.search_results_keyboard(season_id=1, field=field, candidates=[])
        .inline_keyboard[-2][0]
        .callback_data
    )
    confirmation_back = superadmin_hall_of_fame_kb.HallOfFameConfirmCallback.unpack(
        superadmin_hall_of_fame_kb.confirmation_keyboard(season_id=1, field=field, player_id=10)
        .inline_keyboard[-2][0]
        .callback_data
    )

    assert season_back.action == superadmin_hall_of_fame_kb.HallOfFameCardAction.BACK
    assert menu_back.action == superadmin_hall_of_fame_kb.HallOfFameCardAction.BACK_HUB
    assert date_back.action == superadmin_hall_of_fame_kb.HallOfFameDateAction.BACK
    assert name_back.action == superadmin_hall_of_fame_kb.HallOfFameSearchAction.BACK_DATE
    assert results_back.action == superadmin_hall_of_fame_kb.HallOfFameSearchAction.BACK_NAME
    assert confirmation_back.action == superadmin_hall_of_fame_kb.HallOfFameConfirmAction.BACK
...
async def test_superadmin_tournament_hub_open_shows_empty_list(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    planning_service = SimpleNamespace(
        get_superadmin_tournament_hub=AsyncMock(
        ...
        list_open_tournaments_for_superadmin=AsyncMock(
            return_value=Page(items=[], page=0, page_size=6, total_items=0)
        ),
    )
    monkeypatch.setattr(
        superadmin_tournament_handlers,
    ...
    await superadmin_tournament_handlers.select_tournament_hub_action(
        callback,
        superadmin_tournaments_kb.SuperadminTournamentHubCallback(
            action=superadmin_tournaments_kb.SuperadminTournamentHubAction.OPEN
        ),
        state,
    )
...
async def test_superadmin_open_tournaments_list_is_paginated(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    items = [
        SuperadminOpenTournamentListItemView(
            tournament=TournamentView(
                ...
                tournament_type_code="mystery_bounty",
            )
        )
    ]
    planning_service = SimpleNamespace(
        list_open_tournaments_for_superadmin=AsyncMock(
            return_value=Page(items=items, page=1, page_size=1, total_items=3)
        )
    )
    monkeypatch.setattr(
        superadmin_tournament_handlers,
    ...
    await superadmin_tournament_handlers.select_open_tournament_action(
        ...
        superadmin_tournaments_kb.SuperadminOpenTournamentCallback(
            action=superadmin_tournaments_kb.SuperadminOpenTournamentAction.PAGE,
            page=1,
        ),
        state,
    )
...
async def test_superadmin_open_tournament_card_shows_readiness(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    readiness = TournamentCloseReadinessView(
        tournament=TournamentView(
    ...
    await superadmin_tournament_handlers.select_open_tournament_action(
        callback,
        superadmin_tournaments_kb.SuperadminOpenTournamentCallback(
            action=superadmin_tournaments_kb.SuperadminOpenTournamentAction.OPEN,
            page=1,
            tournament_id=12,
        ),
        state,
    ...
    assert message.edit_text.await_args.args[0] == (
        ...
        "• Фото не добавлены.\n\n"
        "Результаты, фото и комбинации редактируются в меню администратора."
    )
    assert inline_keyboard_texts(message.edit_text.await_args.kwargs["reply_markup"]) == [
        "🗑 Удалить игрока",
        "🔒 Закрыть турнир",
...
async def test_superadmin_open_tournament_card_back_returns_list(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    planning_service = SimpleNamespace(
        list_open_tournaments_for_superadmin=AsyncMock(
            return_value=Page(items=[], page=1, page_size=6, total_items=0)
        )
    )
    monkeypatch.setattr(
        superadmin_tournament_handlers,
    ...
    await superadmin_tournament_handlers.select_open_tournament_action(
        callback,
        superadmin_tournaments_kb.SuperadminOpenTournamentCallback(
            action=superadmin_tournaments_kb.SuperadminOpenTournamentAction.BACK_TO_LIST,
            page=1,
            tournament_id=12,
        ),
        state,
...
async def test_superadmin_tournament_hub_closed_shows_closed_list(
    ...
    await superadmin_tournament_handlers.select_tournament_hub_action(
        callback,
        superadmin_tournaments_kb.SuperadminTournamentHubCallback(
            action=superadmin_tournaments_kb.SuperadminTournamentHubAction.CLOSED
        ),
        state,
    )
...
async def test_superadmin_open_tournament_close_uses_existing_close_flow(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    readiness = TournamentCloseReadinessView(
        tournament=TournamentView(
    ...
    await superadmin_tournament_handlers.select_open_tournament_action(
        callback,
        superadmin_tournaments_kb.SuperadminOpenTournamentCallback(
            action=superadmin_tournaments_kb.SuperadminOpenTournamentAction.CLOSE,
            page=2,
            tournament_id=12,
        ),
        state,
    ...
    assert message.edit_text.await_args.args[0] == (
        "Введите фонд турнира для 26.08 — Mystery Bounty"
    )
    assert state.data["close_tournament_id"] == 12
    assert state.data["close_tournament_page"] == 2
    assert state.data["close_return_context"] == "superadmin_open_tournaments"
    assert state.data["close_return_page"] == 2


async def test_close_flow_back_with_open_context_returns_open_list(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    planning_service = SimpleNamespace(
        list_open_tournaments_for_superadmin=AsyncMock(
            return_value=Page(items=[], page=2, page_size=6, total_items=0)
        )
    )
    monkeypatch.setattr(
        superadmin_close_handlers,
    ...
    state = MutableState()
    await state.update_data(
        close_return_context="superadmin_open_tournaments",
        close_return_page=2,
    )
    message = SimpleNamespace(edit_text=AsyncMock())
    callback = SimpleNamespace(
...
async def test_admin_panel_registration_requests_button_shows_pending(
    ...
    assert inline_keyboard_texts(message.answer.await_args.kwargs["reply_markup"]) == [
        "📝 Регистрации пользователей (0)",
        "🎲 Регистрации на турниры (0)",
        "❌ Отмена",
    ]


async def test_admin_panel_registration_requests_button_shows_hub_with_counts(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
async def test_registrations_button_shows_hub_when_both_registration_types_exist(
    ...
    assert message.answer.await_args.args[0] == "Регистрации"
    assert inline_keyboard_texts(message.answer.await_args.kwargs["reply_markup"]) == [
        "📝 Регистрации пользователей (2)",
        "🎲 Регистрации на турниры (3)",
        "❌ Отмена",
    ]


async def test_registrations_button_shows_hub_when_user_requests_absent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = SimpleNamespace(
        get_registrations_overview_for_superadmin=AsyncMock(
    ...
    assert inline_keyboard_texts(reply_markup) == [
        "📝 Регистрации пользователей (0)",
        "🎲 Регистрации на турниры (14)",
        "❌ Отмена",
    ]


async def test_registrations_button_shows_hub_when_tournament_registrations_absent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = SimpleNamespace(
        get_registrations_overview_for_superadmin=AsyncMock(
            return_value=registrations_overview(pending_count=4),
        ),
        list_pending_reviews_page_for_superadmin=AsyncMock(),
    )
    monkeypatch.setattr(superadmin_registration_handlers, "registration_review_service", service)
    ...
    assert message.answer.await_args.args[0] == "Регистрации"
    assert inline_keyboard_texts(message.answer.await_args.kwargs["reply_markup"]) == [
        "📝 Регистрации пользователей (4)",
        "🎲 Регистрации на турниры (0)",
        "❌ Отмена",
    ]


async def test_registrations_hub_empty_user_branch_shows_back(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = SimpleNamespace(
        list_pending_reviews_page_for_superadmin=AsyncMock(
    ...
    await superadmin_registration_handlers.registrations_hub(
        ...
        superadmin_registrations_kb.RegistrationsHubCallback(
            action=superadmin_registrations_kb.RegistrationsHubAction.USER_REQUESTS,
        ),
    )
...
async def test_registrations_hub_empty_tournament_branch_shows_back(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = SimpleNamespace(
        get_registrations_overview_for_superadmin=AsyncMock(
...
async def test_registrations_hub_back_rereads_counts(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = SimpleNamespace(
        get_registrations_overview_for_superadmin=AsyncMock(
            return_value=registrations_overview(pending_count=3, tournament_count=7),
        ),
    )
    monkeypatch.setattr(superadmin_registration_handlers, "registration_review_service", service)
    message = SimpleNamespace(edit_text=AsyncMock())
    ...
    await superadmin_registration_handlers.registrations_hub(
        callback,
        superadmin_registrations_kb.RegistrationsHubCallback(
            action=superadmin_registrations_kb.RegistrationsHubAction.BACK,
        ),
    )
...
async def test_registrations_hub_opens_tournament_counts_with_back(
    ...
    assert "tournament_registrations:open:1" in reply_markup.inline_keyboard[0][0].callback_data
...
def test_registration_list_keyboard_hides_pagination_for_one_pending() -> None:
    ...
    assert inline_keyboard_texts(keyboard) == ["Игрок 10", "⬅️ Назад", "❌ Отмена"]
