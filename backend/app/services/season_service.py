...
from app.db.models.enums import AdminPromptStatus, SeasonStatus, UserRole, UserStatus
...
from app.services.dto import ScoringConfigView, SeasonProposalView, SeasonStatusView, SeasonView
...
class SeasonService:
    ...
    async def create_season_proposal(
        ...
        starts_at = (today or date.today()) + timedelta(days=1)
        async with self.session_factory() as session:
            await self._require_admin(session, admin_telegram_id)
            scoring_config = await self._default_scoring_config(session)
            ...
            return season_proposal_view(proposal)

    async def get_season_proposal(self, prompt_id: int) -> SeasonProposalView:
        async with self.session_factory() as session:
            prompt = await self._get_pending_season_proposal(session, prompt_id)
            return season_proposal_view(prompt)
    ...
    async def _create_proposed_season_in_session(
        ...
        if await ScoringConfigRepository(session).get_by_id(scoring_config_id) is None:
            raise SeasonScoringConfigNotFoundError
        if await repository.get_active() is not None:
    ...
    @staticmethod
    async def _default_scoring_config(session: AsyncSession) -> ScoringConfig | None:
        active_season = await SeasonRepository(session).get_active()
        if active_season is not None:
            return await ScoringConfigRepository(session).get_by_id(
                active_season.scoring_config_id
            )
    ...
    async def _validate_season_start_date(
        ...
        active_season = await SeasonRepository(session).get_active()
        if active_season is not None and starts_at <= active_season.starts_at:
            raise SeasonStartDateError
